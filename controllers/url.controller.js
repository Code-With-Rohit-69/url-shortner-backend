...
const createShortUrl = async (req, res) => {
    ...
    try {
        if (url.trim().length == 0 || !isValidUrl(url)) {
            return res.status(400).json({ message: "Please Provide a valid url.", success: false });
        }

        const existsUrl = await Url.findOne({ originalUrl: url });

        if (existsUrl) {
            return res.status(200).json({ message: "Url already exists", success: true, url: existsUrl.shortCode })
        }
